...
def get_value_dictionary(lista):
    ...
    for item in lista:
        ...
        if len(values) > 1:
            value = item.split('<span class="field-content">')[1].split('</span>')[0].strip()
        else:
            value = item.split('<div class="field-content">')[1].split('</div>')[0].strip()
...
def parse_attachement(attachement_rows):
    ...
    for row in attachement_rows:
        key_extras = row.split('<div class="field-content">')[1].split('</div>')[0]
...
def parse_rows(table):
    ...
    for row in table:
        ...
        if len(value_wrapped.split('<div class="field-item even">')) > 1:
            value_extras = value_wrapped.split('<div class="field-item even">')[1].split('</div>')[0]
        else:
            value_extras = value_wrapped.split('<div class="field-item">')[1].split('</div>')[0]
...
def get_dictionary(file_handler):
    ...
    first_table_rows = string.split('<div class="field-label">')[1:-1] + [string.split('<div class="field-label">')[-1].split('<div class="attachment attachment-after">')[0]]
    dictionary = parse_rows(first_table_rows)
    dictionary['Name'] = title
    dictionary['Country'] = country
    attachement = string.split('<div class="attachment attachment-after">')[1].split('<aside class="grid-6 region region-sidebar-second" id="region-sidebar-second">')[0]

    if  '<div class="view-header">' in attachement:
        attachement_title = attachement.split('<h3>')[1].split('</h3>')[0]
        attachement_rows = attachement.split('<div class="views-field views-field-field-country views-accordion-header">')[1:]
